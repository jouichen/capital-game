    ...
    CHILD: { 
        ...
        themes: {
            ASIA: { 
                n: "🗾 亞洲進階特訓 (超多陷阱)", 
                text: "任務：聽從國家隊指令抓首都！", 
                pool_squirrel: [{e:'🇹🇼 台北', t:true}, {e:'🇯🇵 東京', t:true}, {e:'🇰睿 首爾', t:true}, {e:'🏯 大阪', t:false}, {e:'🍣 京都', t:false}, {e:'🏖️ 普吉島', t:false}], // 東亞正統與混淆城市
                pool_rabbit: [{e:'🇻🇳 河內', t:true}, {e:'🇵🇭 馬尼拉', t:true}, {e:'🇲🇾 吉隆坡', t:true}, {e:'☕ 胡志明市', t:false}, {e:'🏙️ 長灘島', t:false}, {e:'🦁 新加坡', t:true}], // 東南亞
                c: '#2563eb' 
            },
            EUROPE: { 
                n: "🏰 歐洲與世界大戰", 
                text: "任務：考驗國際觀與反射神經！", 
                pool_squirrel: [{e:'🇫🇷 巴黎', t:true}, {e:'🇬🇧 倫敦', t:true}, {e:'🇩🇪 柏林', t:true}, {e:'🇮🇹 羅馬', t:true}, {e:'🍟 鐵塔', t:false}, {e:'🍕 米蘭', t:false}], // 歐洲真假
                pool_rabbit: [{e:'🇺🇸 華盛頓', t:true}, {e:'🇨🇦 渥太華', t:true}, {e:'🇦🇺 堪培拉', t:true}, {e:'🇹🇷 安卡拉', t:true}, {e:'🗽 紐約', t:false}, {e:'🦘 雪梨', t:false}, {e:'🕌 伊斯坦堡', t:false}], // 經典美加澳土耳其超級陷阱！
                c: '#1d4ed8' 
            }
        }
    },
...
function updateModeLogic() {
    ...
    if (currentModeType === 'TODDLER') {
        ...
        levelInd.innerText = "🎯 指令：認準真正的首都狂點！";
    } else {
        ...
        if (currentModeType === 'CHILD') {
            ...
            if (currentThemeKey === 'EUROPE') {
                inst.innerHTML = isSquirrel ? "🔴 【歐洲名城】指令：抓出 🏰 歐洲各國首都！" : "🔵 【西方大國】指令：抓出 🦅 美/加/澳/土耳其首都！";
            }
            levelInd.innerText = "⚠️ 警告：雪梨、紐約、胡志明市、伊斯坦堡都是假首都！";
            activePool = isSquirrel ? themeData.pool_squirrel : themeData.pool_rabbit;
        } else {
            // 養生班
            if (currentThemeKey === 'ASIA') inst.innerHTML = isSquirrel ? "🧘 請收集：東亞首都（如 " + themeData.pool_squirrel[0].e.split(' ')[1] + "）" : "🧘 請收集：東南亞首都（如 " + themeData.pool_rabbit[0].e.split(' ')[1] + "）";
            if (currentThemeKey === 'WORLD') inst.innerHTML = isSquirrel ? "拉開警報 🚨 美國首都是：" + themeData.pool_squirrel[0].e.split(' ')[1] : "拉開警報 🚨 澳洲首都是：" + themeData.pool_rabbit[0].e.split(' ')[1];
            levelInd.innerText = "🧘 輕鬆舒壓，看仔細再點。";
            activePool = isSquirrel ? themeData.pool_squirrel : themeData.pool_rabbit;
        }
    }
}
